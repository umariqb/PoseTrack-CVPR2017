...
public:
    ...
    void setMemoryLimit(const value_type);
...
template<class T>
inline void
Gurobi<T>::setMemoryLimit(
    const value_type gigaByte
) {
    gurobiEnvironment_.set(GRB_DoubleParam_NodefileStart, gigaByte);
    std::cout<<"Setting Memory Limit to "<<gigaByte<<"GBs"<<std::endl;
}
...
Gurobi<T>::optimize() {
    ...
    std::cout<<"Optimization Status = "<<gurobiModel_->get(GRB_IntAttr_Status)<<std::endl;
	double ObjValue = gurobiModel_->get(GRB_DoubleAttr_ObjVal);
	std::cout<<"total cost= "<<ObjValue<<std::endl;
}
