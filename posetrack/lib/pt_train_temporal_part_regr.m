function  pt_train_temporal_part_regr(expidx, cidx)

fprintf('train_temporal_part_regr()\n');
...
modelFname = [ptPairwiseDir '/temporal_model_with_dist_cidx_' num2str(cidx)];

try
    %assert(false);
    load(modelFname,'temporal_model');
    fprintf('cidx: %d - %d\n',cidxs);
    fprintf('temporal model file loaded. quitting.\n');
catch
    [X_pos, X_neg] = pt_get_temporal_features_dm(expidx,cidx);
